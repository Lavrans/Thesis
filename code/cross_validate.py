...
dates = {
    1: {
        'train': ('2011-01-01', '2014-01-01'),
        'val': ('2014-01-01', '2014-02-01'),
        'test': ('2014-02-01', '2014-03-01'),
    },
    2: {
        'train': ('2014-03-01', '2017-03-01'),
        'val': ('2017-03-01', '2017-04-01'),
        'test': ('2017-04-01', '2017-05-01'),
    },
    3: {
        'train': ('2017-05-01', '2020-05-01'),
        'val': ('2020-05-01', '2020-06-01'),
        'test': ('2020-06-01', '2020-07-01'),
    },
    4: {
        'train': ('2020-07-01', '2023-07-01'),
        'val': ('2023-07-01', '2023-08-01'),
        'test': ('2023-08-01', '2023-09-01'),
    },
}
...
def main() -> None:
    ...
    tft = TFT(16, 5, 0.1301, 4, 2)
    tft.to(device)
    tft_optimizer = Adam(tft.parameters(), lr=0.002784)
    tft_scheduler = ExponentialLR(optimizer=tft_optimizer, gamma=0.9649)
    ...
    train_from, train_to = dates[dataset]['train']
    val_from, val_to = dates[dataset]['val']
    test_from, test_to = dates[dataset]['test']

    train_from, train_to = (
        datetime.strptime(train_from, '%Y-%m-%d').date(),
        datetime.strptime(train_to, '%Y-%m-%d').date(),
    )
    val_from, val_to = datetime.strptime(val_from, '%Y-%m-%d').date(), datetime.strptime(val_to, '%Y-%m-%d').date()
    test_from, test_to = datetime.strptime(test_from, '%Y-%m-%d').date(), datetime.strptime(test_to, '%Y-%m-%d').date()
    ...
    tft_train_loader, tft_val_loader, tft_test_loader = get_tft_dataloaders(from_dates, to_dates, 140)
    ...
    bs = BS()
    bs.to(device)
    s, t = get_mlp_data(test_from, test_to)
    s = tensor(s)
    t = tensor(t)
    bs_preds = bs(s)
    bs_loss = sqrt((t - bs_preds) ** 2)
    ...
    mlp_artifact = wandb.Artifact(f'mlp_model_{dataset}', type='model')
    mlp_artifact.add_file('mlp.pt')
    lstm_artifact = wandb.Artifact(f'lstm_model_{dataset}', type='model')
    lstm_artifact.add_file('lstm.pt')
    tft_artifact = wandb.Artifact(f'tft_model_{dataset}', type='model')
    ...
    data = stack(
        [t.to('cpu'), mlp_preds.to('cpu'), lstm_preds.to('cpu'), tft_preds.to('cpu'), bs_preds.to('cpu')]
    ).T.detach()

    df = DataFrame(data=data.to('cpu'), columns=['Target', 'MLP', 'LSTM', 'TFT', 'BS'])

    table = wandb.Table(dataframe=df)

    predictions = wandb.Artifact(f'Predictions_{dataset}', type='predictions')
