...
train_underlying, train_data, train_targets = get_tft_data(date(2012, 1, 1), date(2015, 1, 1))
val_underlying, val_data, val_targets = get_tft_data(date(2015, 1, 1), date(2015, 2, 1))
...
rescale_factor = train_targets.max() - train_targets.min()
target_min = train_targets.min()

train_targets = (train_targets - target_min) / (rescale_factor)
...
def evaluate_one_epoch(model: ScriptModule, val_loader: DataLoader) -> float:
    ...
    with no_grad():
        for idx, (r, s, t) in enumerate(val_loader):
            pred, _, _ = model(s, r)
            pred = pred.squeeze() * rescale_factor + target_min
            loss = criterion(pred, t)
            item = loss.item()
            if not isnan(item):
                val_loss += item
...
def main() -> None:
    wandb.init(project='tft', config=config)
